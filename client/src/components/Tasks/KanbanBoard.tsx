    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {COLUMNS.map((col) => {
        ...
        return (
          <div
            key={col.id}
            className={`rounded-2xl border ${col.accent} bg-white/3 flex flex-col`}
          >
            {/* Column header */}
            <div
            ...
            {/* Cards */}
            <div className=" p-3 space-y-4 flex-1 overflow-y-scroll custom-scrollbar max-h-[60vh] scroll">
              {colTasks.length === 0 ? (
                <EmptyState
                  icon={CheckSquare}
                  title="No tasks here"
